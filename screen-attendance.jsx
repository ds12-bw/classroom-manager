...
const Attendance = ({activeClass, setActiveClass, onNav, setActiveStudent}) => {
  ...
  // Load attendance from store — empty object means "not yet recorded".
  // Don't useMemo — store.attendance is mutated in place; recompute on every render.
  const statuses = window.getAttendance(cls.id, date) || {};

  // Has any attendance been recorded for this date?
  const hasRecord = Object.keys(statuses).length > 0;

  const counts = (() => {
    const c = {present:0, absent:0, leave:0, skip:0, unmarked:0};
    roster.forEach(s => {
  ...
  const filtered = roster.filter(s => {
    ...
    if (query && !(s.name + s.surname + s.id).includes(query)) return false;
    return true;
  });

  const setStatus = (sid, status) => {
    window.setAttendance(cls.id, date, sid, status);
  };

  const setAllPresent = () => {
    const all = {...statuses};
    roster.forEach(s => { if (!all[s.id]) all[s.id] = "present"; });
    window.setAttendanceBulk(cls.id, date, all);
  };

  const resetAll = () => {
    if (!confirm("ล้างการเช็คชื่อของวันนี้ทั้งหมด?")) return;
    window.clearAttendance(cls.id, date);
  };
  ...
  return (
    <div className="main fade-in">
      <PageHead title="เช็คชื่อนักเรียน"
        ...
        right={<>
          {hasRecord && <span className="badge" style={{background:"#D1FAE5", color:"#065F46"}}>บันทึกอัตโนมัติ ✓</span>}
          <button className="btn btn-ghost" onClick={resetAll} disabled={!hasRecord}><Icon name="edit" size={14}/> ล้าง</button>
          <button className="btn btn-primary" onClick={setAllPresent}><Icon name="check" size={14}/> มาทั้งหมด</button>
        </>}
      />
      ...
      <div className="page-body" style={{display:"flex", flexDirection:"column", gap:18}}>
        ...
        <div style={{display:"grid", gridTemplateColumns:"repeat(5, 1fr)", gap:12}}>
          {[
            ...
            {k:"unmarked", count:counts.unmarked, label:"ยังไม่เช็ค"},
          ].map(s => {
            const st = window.ATT_STATUS[s.k] || {color:"#9CA3AF", bg:"#F3F4F6", letter:"-"};
        ...
        <div className="row">
          <SearchBox value={query} onChange={setQuery} placeholder="ค้นหาชื่อ / รหัส" />
          <div className="tgrp">
            ...
            <button className={filter==="unmarked" ? "on" : ""} onClick={()=>setFilter("unmarked")}>ยังไม่เช็ค</button>
          </div>
        </div>

        {/* Student list — single column, sorted by no, big tap targets */}
        {roster.length === 0 ? (
          <div className="card card-pad-lg" style={{textAlign:"center", padding:40}}>
            <div className="muted">ห้องนี้ยังไม่มีนักเรียน เพิ่มที่หน้ารายชื่อก่อน</div>
          </div>
        ) : (
        <div style={{display:"flex", flexDirection:"column", gap:8}}>
          {filtered.map(s => {
            const status = statuses[s.id];
            const stMeta = status ? window.ATT_STATUS[status] : {color:"#D1D5DB", bg:"#F3F4F6", letter:"-", label:"ยังไม่เช็ค"};
            return (
              <div key={s.id} className="card" style={{padding:"10px 14px", display:"flex", gap:14, alignItems:"center", borderLeft:`5px solid ${stMeta.color}`, background: status ? "#fff" : "#FAFAF8"}}>
                <div className="num bold" style={{width:36, textAlign:"center", fontSize:18, color:"var(--ink-3)"}}>{s.no}</div>
                <button onClick={()=>{ setActiveStudent(s.id); onNav("student"); }} style={{display:"flex", alignItems:"center", gap:12, flex:1, textAlign:"left", minWidth:0}}>
                  <Avatar color={s.avatar} label={s.name.slice(0,1)} />
                  <div style={{minWidth:0, flex:1}}>
                    <div className="bold" style={{fontSize:15, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis"}}>
                      {s.prefix}{s.name} {s.surname}
                    </div>
                    <div className="muted text-sm num">{s.id} {!status && <span style={{color:"#9CA3AF"}}>· ยังไม่เช็ค</span>}</div>
                  </div>
                </button>
                  ...
                  {["present","absent","leave","skip"].map(k => {
                    ...
                    return (
                      <button key={k} title={sm.label}
                        ...
                        style={{
                          minWidth:54, height:44, borderRadius:10, fontWeight:700,
                          background: on ? sm.color : sm.bg,
                          color: on ? "#fff" : sm.color,
                          fontSize:13, padding:"0 10px",
                          display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center",
                          border: on ? `2px solid ${sm.color}` : "2px solid transparent",
                          transition:"all .12s"
                        }}>
                        <span style={{fontSize:16, lineHeight:1}}>{sm.letter}</span>
                        <span style={{fontSize:10, marginTop:2, opacity:.85}}>{sm.label}</span>
                      </button>
                    );
                  })}
                </div>
...
const Roster = ({activeClass, setActiveClass, onNav, setActiveStudent, onImportStudents}) => {
  ...
  const [adding, setAdding] = useState(false);
  const [draftNew, setDraftNew] = useState({prefix:"เด็กชาย", name:"", surname:"", id:""});
  const [editingId, setEditingId] = useState(null);

  const submitNew = () => {
    if (!draftNew.name.trim()) return;
    window.addStudent({...draftNew, classId: cls.id});
    setDraftNew({prefix:"เด็กชาย", name:"", surname:"", id:""});
    setAdding(false);
  };
      ...
      <div className="page-body" style={{display:"flex", flexDirection:"column", gap:18}}>
        ...
        {adding && (
          <div className="card card-pad-lg" style={{background:"linear-gradient(135deg, #EEF0FF 0%, #FCE7F3 100%)", border:"none"}}>
            ...
            <div style={{display:"grid", gridTemplateColumns:"100px 130px 110px 1fr 1fr auto", gap:10, alignItems:"end"}}>
              <div>
