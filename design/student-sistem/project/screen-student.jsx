...
const StudentDetail = ({activeStudent, onNav, activeClass}) => {
  ...
  const [editingField, setEditingField] = useState(null); // "name" | "nick" | "id"
  ...
  const hist = useMemo(() => {
    const r = []; let seed = parseInt(s.id) % 1000;
    for (let i=0;i<28;i++){
      seed = (seed*1103515245 + 12345) % 0x7fffffff;
      const v = seed % 100;
      const day = i % 7;
      if (day === 5 || day === 6){ r.push(null); continue; }
      let st = "present";
      if (v < 8) st = "absent";
      else if (v < 12) st = "leave";
      else if (v < 16) st = "skip";
      r.push(st);
    }
    return r;
  }, [s.id]);
  ...
  return (
    <div className="main fade-in">
      <PageHead
        title={<span>{window.studentShortName(s)} <span className="muted" style={{fontWeight:400}}>({s.nick})</span></span>}
      ...
      <div className="page-body" style={{display:"grid", gridTemplateColumns:"320px 1fr", gap:20}}>
        ...
        <div className="col">
          <div className="card card-pad-lg" style={{textAlign:"center"}}>
            <Avatar color={s.avatar} label={s.nick.slice(0,1)} size={84}/>
            <div className="bold text-lg" style={{marginTop:14}}>{window.studentFullName(s)}</div>
            <div className="muted text-sm">{s.nick} · {cls.name}</div>
          ...
          <div className="card">
            ...
            <div className="col" style={{gap:10}}>
              ...
              <div>
                ...
                <input className="input" value={s.nick} style={{width:"100%"}}
                  onChange={e=>window.updateStudent(s.id, {nick: e.target.value})}/>
              </div>
              <div>
        ...
        <div className="col">
          ...
          <div className="card card-pad-lg">
            ...
            <div className="row" style={{gap:14, marginBottom:18}}>
              {Object.entries(s.attendance).map(([k,v]) => {
...
const ImportModal = ({onClose, kind = "students", targetClassId = null}) => {
  ...
  const subtitle = isClasses
    ...
    : "รองรับ .xlsx / .xls / .csv · คอลัมน์: รหัส, ชื่อ-สกุล, ชื่อเล่น";
          ...
          {step === "upload" && (
            <div>
              ...
              {isClasses ? (
                ...
                <div className="card" style={{marginTop:16, background:"var(--bg)"}}>
                  <div className="bold text-sm" style={{marginBottom:8}}>รูปแบบไฟล์ที่แนะนำ</div>
                  <div style={{display:"grid", gridTemplateColumns:"0.4fr 0.9fr 0.7fr 0.8fr 0.8fr 0.6fr", gap:0, border:"1px solid var(--line)", borderRadius:8, overflow:"hidden", fontSize:12}}>
                    {["เลขที่","รหัส","คำนำหน้า","ชื่อ","สกุล","ชื่อเล่น"].map((h,i)=>(
                      <div key={i} style={{padding:"6px 8px", background:"#F3EFE7", borderRight: i<5 ? "1px solid var(--line)" : "none"}} className="bold">{h}</div>
                    ))}
                    {[
                      ["1","6512345","เด็กชาย","สมชาย","ใจดี","โอม"],
                      ["2","6512346","เด็กหญิง","สมหญิง","รักเรียน","หญิง"],
                      ["3","6512347","นาย","ธนกร","ทองดี","กบ"],
                    ].map((row,r) => row.map((cell,i)=>(
                      <div key={`${r}-${i}`} style={{padding:"6px 8px", borderTop:"1px solid var(--line)", borderRight: i<5 ? "1px solid var(--line)" : "none"}} className={i===0||i===1 ? "num" : ""}>{cell}</div>
                    )))}
                  </div>
                  <div className="muted text-sm" style={{marginTop:8}}>คอลัมน์: <span className="bold">เลขที่ · รหัส · คำนำหน้า · ชื่อ · สกุล · ชื่อเล่น</span><br/>ไม่ต้องมีคำนำหน้าก็ได้ ระบบจะแยกจากชื่อให้อัตโนมัติ (เช่น "เด็กชายสมชาย ใจดี")</div>
                </div>
              )}
            </div>
          )}
                      ...
                      <tr key={i}>
                        {isClasses ? (
                          ...
                          <>
                            ...
                            <td>{r.nick || <span className="muted">—</span>}</td>
                          </>
                        )}
                      </tr>
                    ))}
...
const QRDisplay = ({size=220, seed="MATH-401"}) => {
  ...
  const cells = useMemo(() => {
    ...
    for (let r=0;r<N;r++){
      const row=[];
      for (let c=0;c<N;c++){
        const isFinder = (rg, cg) => {
          const dr = Math.max(rg-r, r-(rg+6), 0);
          const dc = Math.max(cg-c, c-(cg+6), 0);
          if (dr===0 && dc===0){
            const ir = r-rg, ic = c-cg;
            if (ir===0 || ir===6 || ic===0 || ic===6) return 1;
            if (ir>=2 && ir<=4 && ic>=2 && ic<=4) return 1;
            return -1;
          }
          return 0;
        };
        let f = isFinder(0,0); if (f===0) f = isFinder(0, N-7); if (f===0) f = isFinder(N-7, 0);
        if (f === 1) { row.push(1); continue; }
        if (f === -1) { row.push(0); continue; }
        h = (h*1103515245 + 12345) & 0x7fffffff;
        row.push(h % 100 < 48 ? 1 : 0);
      }
      grid.push(row);
    }
    return grid;
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{background:"#fff", borderRadius:12}}>
      {cells.map((row, r) => row.map((v, c) => v ? (
        <rect key={`${r}-${c}`} x={c*cs} y={r*cs} width={cs} height={cs} fill="#1E1B2E" rx={cs*0.15}/>
      ) : null))}
    </svg>
  );
};
...
const QRPage = ({activeClass, setActiveClass}) => {
  ...
  const url = `https://kru.app/c/${cls.code.toLowerCase()}`;
  return (
    <div className="main fade-in">
      ...
      <div className="page-body" style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:24}}>
        <div className="col">
          ...
          <div className="card">
            ...
            <ol style={{paddingLeft:20, margin:0, color:"var(--ink-2)", fontSize:13, lineHeight:1.8}}>
              ...
              <li>กรอกเลขประจำตัวนักเรียน 7 หลัก</li>
        ...
        <div className="card card-pad-lg" style={{display:"flex", flexDirection:"column", alignItems:"center", background:"linear-gradient(180deg, #FFFFFF 0%, #FAF7F2 100%)"}}>
          <div style={{padding:24, background:"#fff", borderRadius:24, boxShadow:"var(--shadow-md)", border:`3px solid ${cls.color}`, position:"relative"}}>
            <QRDisplay size={240} seed={cls.code}/>
...
const StudentMobile = ({activeClass}) => {
  ...
  const cats = store.categories;
  const s = store.students.find(x=>x.id===studentId) || store.students[0];
  const sc = store.scores[s.id] || {};
  const maxTotal = window.maxTotal();
  const total = cats.reduce((a,c)=>a+(sc[c.key]||0), 0);
  const pct = maxTotal ? (total/maxTotal)*100 : 0;
      ...
      <div className="page-body" style={{display:"flex", justifyContent:"center", gap:32, alignItems:"flex-start", flexWrap:"wrap"}}>
        ...
        <div className="phone-frame">
          <div className="phone-screen">
            ...
            {step === "login" ? (
              <div style={{padding:"60px 24px 24px", display:"flex", flexDirection:"column", alignItems:"center", height:"100%"}}>
                <div style={{width:64, height:64, borderRadius:16, background:cls.color, display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", fontFamily:"var(--font-num)", fontWeight:700, fontSize:24, marginTop:24}}>
                  {cls.code.split("-")[0].slice(0,2)}
                </div>
                <div className="bold text-lg" style={{marginTop:16}}>{cls.name}</div>
                <div className="muted text-sm">เช็คคะแนน · การเข้าเรียน</div>
                <div className="card" style={{marginTop:32, width:"100%", padding:16}}>
                  ...
                  <div className="muted text-sm" style={{marginBottom:12}}>7 หลัก ที่ขึ้นต้นด้วย 65</div>
                  <input className="input num" style={{width:"100%", fontSize:18, letterSpacing:2, textAlign:"center"}}
                    placeholder="6500000"
                    value={pin}
                    maxLength={7}
                    onChange={e=>setPin(e.target.value.replace(/\D/g,""))}/>
                  <div className="row" style={{flexWrap:"wrap", gap:6, marginTop:10, justifyContent:"center"}}>
                    {store.students.slice(0,4).map(s=>(
                      <button key={s.id} className="badge" style={{background:"var(--primary-soft)", color:"var(--primary-ink)", cursor:"pointer"}}
                        onClick={()=>setPin(s.id)}>{s.id} ({s.nick})</button>
                    ))}
                  </div>
                  <button className="btn btn-primary" style={{width:"100%", marginTop:16, justifyContent:"center"}}
                    disabled={pin.length<7}
                    onClick={()=>{ setStudentId(pin); setStep("dashboard"); }}>
                    เข้าสู่ระบบ
                  </button>
                </div>
                <div className="muted text-sm" style={{marginTop:"auto", paddingTop:14}}>โดย ครูดิจิทัล · ปลอดภัย ส่วนตัว</div>
              </div>
            ) : (
              <div style={{padding:"50px 16px 24px"}}>
                <div className="row" style={{justifyContent:"space-between", marginBottom:14}}>
                  <button onClick={()=>{ setStep("login"); setPin(""); }} className="btn btn-ghost" style={{padding:"6px 10px"}}>
                ...
                <div style={{padding:18, borderRadius:18, background:`linear-gradient(135deg, ${cls.color} 0%, ${cls.color}cc 100%)`, color:"#fff", marginBottom:14}}>
                  <div className="row" style={{gap:12}}>
                    <Avatar color="#fff" label={s.nick.slice(0,1)} size={48}/>
                    <div>
                      <div className="bold" style={{fontSize:16}}>{s.name}</div>
                      <div className="text-sm" style={{opacity:.9}}>{s.nick} · เลขที่ {s.no}</div>
                    </div>
                  </div>
                  <div className="row" style={{justifyContent:"space-around", marginTop:16, paddingTop:14, borderTop:"1px solid rgba(255,255,255,.2)"}}>
                    <div style={{textAlign:"center"}}>
                      <div className="num bold" style={{fontSize:24}}>{total}<span className="text-sm" style={{opacity:.8}}>/{maxTotal}</span></div>
                      <div className="text-sm" style={{opacity:.9}}>คะแนนรวม</div>
                    </div>
                    <div style={{textAlign:"center"}}>
                      <div className="num bold" style={{fontSize:24}}>{grade}</div>
                      <div className="text-sm" style={{opacity:.9}}>เกรด</div>
                    </div>
                    <div style={{textAlign:"center"}}>
                      <div className="num bold" style={{fontSize:24}}>{attPct}%</div>
                      <div className="text-sm" style={{opacity:.9}}>เข้าเรียน</div>
                    </div>
                  </div>
                </div>
          ...
          <div className="card card-pad-lg">
            ...
            <div className="col" style={{gap:14}}>
              {[
                ...
                {n:2, t:"กรอกเลขประจำตัว 7 หลัก", d:"ระบบจะยืนยันก่อนแสดงข้อมูล"},
